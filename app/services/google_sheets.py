...
class GoogleSheetsService:
    ...
    def create_sheet(self, spreadsheet_name: str, sheet_title: str):
        ...
        permission = {
            "role": "writer",
            "type": "user",
            "emailAddress": "cheng94orange@gmail.com"
        }

        # alternatively, if want to give permission to all users, just change permission to this:
        # permission = {
        #     "role": "writer",
        #     "type": "anyone",
        # }
    ...
    def read_worksheet(self, spreadsheet_id:str, worksheet_name: str):
        #to select data in googlesheet of column A row 1 and column B row 1 is: spreadsheet_name!A1:B1
        range_name = f"{worksheet_name}!A1:ZZZ" # the maximum number of column of googlesheet is ZZZ
    ...
    def append_records(self, spreadsheet_id: str, worksheet_name: str, records: List[Dict[str, Any]]):
        ...
        for record in records:
            ...
            record_new_headers = [key for key in record.keys() if key not in existing_headers]
            new_headers.update(record_new_headers)

            #update the headers if the new columns were added
        ...
        request_body = {"values": new_records}
    ...
    def append_records_autoincrement(
            self, spreadsheet_id: str, worksheet_name: str, records:List[Dict[str, Any]]
    ):
        existing_data = self.read_worksheet(spreadsheet_id, worksheet_name)
        max_id = 0
        for row in existing_data:
